...
// 发音功能（兼容手机）
function speakWord() {
  var word = wordList[currentIndex].english;
  ...
  // 方法2：使用有道词典在线发音（无需API，免费）
  playOnlineAudio(word);
}

// 使用有道在线发音
function playOnlineAudio(word) {
  var audio = new Audio();
  audio.src = 'https://dict.youdao.com/dictvoice?audio=' + encodeURIComponent(word) + '&type=0';
  audio.play().catch(function() {
    // 如果上面也不行，尝试备用链接
    var audio2 = new Audio();
    audio2.src = 'https://dict.youdao.com/dictvoice?audio=' + encodeURIComponent(word) + '&type=1';
    audio2.play().catch(function() {
      alert("🔊 请在电脑上使用发音功能，或复制单词到词典APP查看发音");
    });
  });
}
